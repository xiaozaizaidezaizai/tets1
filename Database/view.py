...
# 功能：删除 launch, votedata, voterecords表captcha的相关记录
...
def showVoteResult(captcha, total, prikey):
    ...
    # 返回参数：
    '''
    data = getVoteData(captcha)#在votedata表中返回captcha记录对应 (choice, tag) 选项和素数
    choices = []
    tags = []
    for x in data:
        choices.append(x[0])
    ...
    for i in range(len(tags)):
        c = 0
        while total % tags[i] == 0:#对素数取余
            c = c + 1
            total = total // tags[i] #整除向下取整
        cnt.append(c)
    res = []
    for i in range(len(tags)):
...
if __name__ == "__main__":
    ...
    prikey = load_key('../key/PR_1.txt', 1)
    total = (
    ,
